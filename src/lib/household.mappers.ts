...
export function mapSplitClaim(r: Raw): SplitClaim {
  ...
  return {
    ...
    shareAmount,
    settledAmount,
    offsetAmount: Number(r.offset_amount ?? 0),
...
export function mapSettlement(r: Raw): Settlement {
  return {
    ...
    offsetTotal: Number(r.offset_total ?? 0),
    currency: String(r.currency ?? 'MYR'),
    note: String(r.note ?? ''),
    scopeFrom: r.scope_from ? String(r.scope_from) : null,
    scopeTo: r.scope_to ? String(r.scope_to) : null,
