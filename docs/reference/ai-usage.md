...
| P2 | **Photo import** — receipt or statement screenshot → transaction draft(s), feeding the CSV review table | TBD | Haiku | One call per uploaded photo (a multi-select batch of N photos is N calls, N rate-limit units — accepted tradeoff, see the spec), explicit user action, never automatic. Rows land in the **same review-and-confirm table** CSV import uses — never a silent write. No key → entry point hidden. Approved 2026-09-06. Full spec: [../wallet/feature-photo-import.md](../archive/design-adoption/feature-photo-import.md). |
| A4 | **CSV import review — "Ask AI to suggest"** for rows the rules pass left uncategorised | — | Haiku | Reuses `suggestCategoriesAI`/`suggest-categories-ai` exactly (the same call `BulkEditDialog`'s "Ask AI" button already makes) — same chunking, same rate-limit bucket, same THROWS/failedMerchants reporting. Explicit button click only, never automatic on import (that automatic version is P1 below, still declined). No key → button hidden. Approved 2026-09-06. |
| A5 | **CSV import review — "Ask AI to resolve merchant names"** for narrative-split rows the rules ladder left unresolved | — | Haiku | Not a new call — `resolveMerchantsWithAI`/`POST /merchants/resolve` already existed (§1) but ran its Stage 3 (AI) automatically on every CSV import with an unresolved narrative-split merchant, which never went through this register and quietly violated rule 3 ("explicit trigger only"). Split to match A4's shape: `ImportModal.tsx`'s automatic on-import pass now runs `resolveMerchantLadder` with `useAI: false` (Stages 1-2 only — corrections cache + own history, zero AI spend); Stage 3 is reached only via this explicit review-page button, for whatever stays unresolved. Same chunking, same rate-limit bucket (`ai_rate_limit_merchant`), same messaging shape as A4. No key → button hidden. Approved 2026-09-06 (retroactively covers the pre-existing automatic call, which is removed by this same change). |
...
|---|---|---|---|---|
| P1 | **CSV per-row auto-categorisation on import** (§9.3 step 6) — suggest a category for each non-duplicate imported row | R7/R8 | Haiku | Reuses `suggestCategoriesAI` exactly. Batched: **one call for the whole import**, not one per row. Recommended, because the machinery already exists and import is the highest-volume categorisation moment — but it is a new *automatic* call at import time, so it needs your yes. **Declined 2026-09-06**: the rules pass (history + builtin map) already resolves most rows for free; spending an AI call on every import — including rows a human would never need to ask about — isn't "needed" per rule 3. A4 above covers the actual gap (genuinely new merchants) as an explicit manual action instead. |
...
| 2026-09-06 | P2 photo import | **approved** — confirmed in chat, proceed to wire exactly as scoped in [../wallet/feature-photo-import.md](../archive/design-adoption/feature-photo-import.md) |
| 2026-09-06 | A5 merchant-resolution explicit trigger | **approved** — confirmed in chat ("go ahead with PR1 and PR2"), after the owner asked why merchant resolution didn't mirror A4's rules-first/explicit-AI split; this PR makes it match |
