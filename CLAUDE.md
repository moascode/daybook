...
Next task:      REVIEW + MERGE the dark mode PR (branch
                claude/dark-mode-r2ihp9). No migration and no server change
                — it is a client-only diff, but a WIDE one (64 files), so
                the useful review is the token layer in
                scripts/gen-theme-tokens.mjs + tailwind.config.js and a
                click through both themes; the per-file churn is a codemod.
                Then REVIEW + MERGE PR #95 (Shared review flow R1-R3). R2
                rewrites twelve money-path
